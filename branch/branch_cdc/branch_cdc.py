...
class DBConnection:
    ...
    def connect(self):
        try:
            # Review Fix: Use autocommit=False to ensure explicit transaction control
...
def ensure_meta_table(ds_conn):
    # Review Fix: Ensure downstream DB exists
...
def acquire_lock(ds_conn, tid):
    if not ds_conn.query(f"SELECT 1 FROM `{META_DB}`.`{META_TABLE}` WHERE task_id = %s", (tid,)):
        ...
        ds_conn.commit()
    # Review Fix: Support lock stealing after 30s
...
def heartbeat_lock(ds_conn, tid):
    # Review Fix: Added lock renewal to prevent timeout during long syncs
    try:
        ds_conn.execute(f"UPDATE `{META_DB}`.`{META_TABLE}` SET lock_time=NOW() WHERE task_id=%s AND lock_owner=%s", (tid, INSTANCE_ID))
        ds_conn.commit()
    except: pass

def release_lock(ds_conn, tid):
    try:
        ds_conn.execute(f"UPDATE `{META_DB}`.`{META_TABLE}` SET lock_owner=NULL WHERE task_id=%s AND lock_owner=%s", (tid, INSTANCE_ID))
        ds_conn.commit()
    except: pass
...
def get_check_sql(conn, db, table, snap=None, sample=False):
    ...
    cols = conn.query(f"SHOW COLUMNS FROM `{db}`.`{table}`")
    ...
    # Review Fix: Use CRC32(CAST(..)) instead of hashtype for compatibility
...
def verify_consistency(up_conn, ds_conn, config, snap=None, sample=False):
    u, d = config["upstream"], config["downstream"]
    try:
        # Review Fix: Implemented true 1% sampling check instead of just count
        sql_u = get_check_sql(up_conn, u['db'], u['table'], snap, sample)
        sql_d = get_check_sql(ds_conn, d['db'], d['table'], None, sample)
        ur, dr = up_conn.fetch_one(sql_u), ds_conn.fetch_one(sql_d)
        if ur['c'] == dr['c'] and ur['h'] == dr['h']:
            mode = "FAST" if sample else "FULL"
            log.info(f"[green]{mode} Check PASSED | Rows: {ur['c']} | Hash: {ur['h']}[/green]"); return True
...
def archeology_recovery(up_conn, ds_conn, config, tid):
    # Review Fix: Implemented "Archeology Mode" to recover watermarks if meta is lost
    log.info("[dim]Archeology Mode: Searching for historical snapshots...[/dim]")
    sid_prefix = hashlib.md5(tid.encode()).hexdigest()[:12]
    snaps = up_conn.query(f"SELECT sname FROM mo_catalog.mo_snapshots WHERE sname LIKE 'cdc_{sid_prefix}_%' ORDER BY ts DESC LIMIT 10")
    for s in snaps:
        snap = s['sname']
        if verify_consistency(up_conn, ds_conn, config, snap, sample=True):
            log.info(f"[green]Watermark recovered: {snap}[/green]")
            ds_conn.execute(f"INSERT INTO `{META_DB}`.`{META_TABLE}` (task_id, watermark) VALUES (%s, %s)", (tid, snap))
            ds_conn.commit(); return snap
    return None
...
def perform_sync(config, is_auto=False):
    ...
    try:
        ...
        # Review Fix: Ensure downstream DB exists before synchronization
        ds_conn.execute(f"CREATE DATABASE IF NOT EXISTS `{d_cfg['db']}`")
        ds_conn.commit()
        ...
        if not ds_conn.query(f"SHOW TABLES FROM `{d_cfg['db']}` LIKE '{d_cfg['table']}'"):
            ...
            ds_conn.execute(f"USE `{d_cfg['db']}`"); ds_conn.execute(ddl); ds_conn.commit()

        ws = get_watermarks(ds_conn, tid)
        lastgood = ws[0] if ws else archeology_recovery(up_conn, ds_conn, config, tid)
        
        # Ensure clean state for next steps
        up_conn.commit()
        ds_conn.commit()
        ...
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), console=console) as prg:
            if not lastgood:
                ...
                up_conn.commit()
                up_conn.execute(f"CREATE TABLE `{u_cfg['db']}`.`{t_zero}` LIKE `{u_cfg['db']}`.`{u_cfg['table']}`")
                up_conn.commit()
                ...
                heartbeat_lock(ds_conn, tid)
                ds_conn.conn.begin()
                try:
                    for r in dfiles:
                        f = list(r.values())[0]
                        slash, quote = chr(92), chr(34)
                        sql = f"LOAD DATA INFILE '{f}' INTO TABLE `{d_cfg['db']}`.`{d_cfg['table']}` FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '{quote}' ESCAPED BY '{slash}{slash}' LINES TERMINATED BY '{slash}n' PARALLEL 'TRUE'"
                        ds_conn.execute(sql)
                    ds_conn.execute(f"INSERT INTO `{META_DB}`.`{META_TABLE}` (task_id, watermark) VALUES (%s, %s)", (tid, newsnap))
                    ds_conn.commit(); sync_success = True
                except Exception as e: ds_conn.rollback(); raise e
            else:
                log.info(f"[blue]INCREMENTAL Sync | Task: {tid} | From {lastgood}[/blue]")
                if not up_conn.query(f"SELECT sname FROM mo_catalog.mo_snapshots WHERE sname = '{lastgood}'"):
                    log.error("Snapshot missing on upstream!"); return False
                
                t_cp = f"{u_cfg['table']}_copy_prev"
                up_conn.execute(f"DROP TABLE IF EXISTS `{u_cfg['db']}`.`{t_cp}`")
                up_conn.commit()
                up_conn.execute(f"data branch create table `{u_cfg['db']}`.`{t_cp}` from `{u_cfg['db']}`.`{u_cfg['table']}`{{snapshot='{lastgood}'}}")
                up_conn.commit()
                up_conn.execute(f"CREATE SNAPSHOT `{newsnap}` FOR TABLE `{u_cfg['db']}` `{u_cfg['table']}`")
                up_conn.commit()
                ...
                heartbeat_lock(ds_conn, tid)
                # Ensure DS is clean before BEGIN
                ds_conn.commit()
                ds_conn.conn.begin()
                try:
                    for r in dfiles:
                        ...
                        # Review Fix: Robustly strip BEGIN/COMMIT and handle INSERT/UPDATE
                        ...
                        # Rewrite all standard DML to target table
                        sql = re.compile(r'(delete\s+from\s+|replace\s+into\s+|insert\s+into\s+|update\s+)([`\w]+\.[`\w]+|[`\w]+)', re.I).sub(lambda m: f"{m.group(1)} {target} ", stmt_str)
        ...
        if sync_success:
            ...
            for r in dfiles:
                # Review Fix: Proper REMOVE syntax
        ...
        if perform_sync(config, is_auto=True):
            ...
            if sc % 5 == 0 or (v_int > 0 and sc % v_int == 0):
                ...
                if up.connect() and ds.connect():
                    ...
                    if ws:
                        # Review Fix: Perform actual sampling hash check
                        is_full = (v_int > 0 and sc % v_int == 0)
                        verify_consistency(up, ds, config, ws[0], sample=not is_full)
                    up.close(); ds.close()
        time.sleep(interval)
